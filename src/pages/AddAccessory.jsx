...
const AddAccessory = ({ user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess(false);
    if (!user) {
      setError('You must be logged in to add an accessory.');
      return;
    }
    ...
    try {
      ...
      setLoading(false);
      setSuccess(true);
      setTimeout(() => navigate('/accessories'), 1200);
    } catch (err) {
      console.error("Error adding accessory: ", err);
      setError('Failed to add accessory. ' + (err.message || ''));
      setLoading(false);
    }
  };
  ...
  return (
    <Paper sx={{ p: { xs: 2, md: 4 }, maxWidth: 600, mx: 'auto', mt: 6, borderRadius: 2, boxShadow: 3 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <TextField label="Accessory Name" value={name} onChange={e => setName(e.target.value)} required />
        <TextField label="Price ($)" type="number" value={price} onChange={e => setPrice(e.target.value)} required />
        <TextField label="Description" value={description} onChange={e => setDescription(e.target.value)} multiline minRows={3} required />
        ...
        {success && <Typography color="success.main">Accessory added!</Typography>}
        <Button type="submit" variant="contained" sx={{ bgcolor: '#FF6B6B', color: '#fff', fontWeight: 700, borderRadius: 2, px: 4, py: 1.5, fontSize: '1.1rem', boxShadow: 'none', '&:hover': { bgcolor: '#e55a5a' } }} disabled={loading}>
          {loading ? <CircularProgress size={24} color="inherit" /> : 'Add Accessory'}
        </Button>
      </Box>
    </Paper>
  );
